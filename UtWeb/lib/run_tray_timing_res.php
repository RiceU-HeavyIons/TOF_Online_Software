<?php
# $Id: run_tray_timing_res.php,v 1.8 2008/05/16 04:40:34 tofp Exp $
...
?>
<div class="section">
<div class="title">Tray Timing Test(ps)</div>
<?php
echo "<table>\n";
echo "<tr>\n";
echo "<th>slot #</th>\n";
...
foreach(ModuleResult::find_all("run_id=" . $rvar_id, "slot", "") as $mr) {
   ...
   for($i=1; $i <= $mr->npad; $i++) {
      echo "  <td style=''>" . tricolor($mr->pad[$i], 300, 400) . "</td>\n";
   }
   echo "  <td style=''>" . tricolor($mr->avg, 300, 400) . "</td>\n";
   echo "  <td style=''>" . tricolor($mr->min, 300, 400) . "</td>\n";
   echo "  <td style=''>" . tricolor($mr->max, 300, 400) . "</td>\n";
   echo " </tr>\n";
}
echo "</table>\n";
?>
