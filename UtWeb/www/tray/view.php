<?php
# $Id: view.php,v 1.17 2012/01/05 17:10:52 tofp Exp $
...
if ($tray->id > 0) {
  ...
  echo "<table>\n";
#  echo "<tr><th>Id</th>\n";
#  echo " <td>", mk_link($tray->id, "./view.php?id=" . $tray->id), "</td></tr>\n";
  echo "<tr><th>Id</th>\n";
  echo " <td>", $tray->id, "</td></tr>\n";

##dylan -- this is the line I'm editing 
$position_D = mysql_query("SELECT position FROM tray_positions where tray_id = $tray->id", $con);
$posD2 = mysql_fetch_object($position_D);
  echo "<tr><th>Position</th>\n";
 echo " <td>", $posD2->position , "</td></tr>\n";
##end damage
...
#tr.row0:hover { color: black; background: #f0f0d0; }
  ...
  while($i>0){
    ...
    if($tray->tdig && ($tdig=$tray->tdig[$i-1])) {
      echo "<tr class='row$ll'>\n";
      echo " <td></td><td></td>\n";
    ...
    $ll=$l%2;
    echo "<tr class='row$ll'>\n";
    if($tray->tino && ($tino=$tray->tino[$i-1])){
      echo " <td></td><td></td>\n";
      echo " <td>Board #$i &rarr;</td>\n";
      echo " <td class='tino'><a href=\"{$action}?id=", $tino->id, "\">", $tino->sn, "</td>\n";
      echo "</tr>\n";
    } else {
      echo "<td></td><td></td><td></td><td>Nothing here </td></tr>";
    }
    echo "</tr>\n";
