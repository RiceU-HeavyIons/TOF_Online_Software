...
#define FORMAT_VERSION      0x22              /* Format Version number for trigger data */
...
} RawTrgDet;           /* 1792 bytes total */

/*  Trigger Event Structure */

typedef struct {
  EvtDescData    EvtDesc;               /* L1 Event Descriptor Data : 40 bytes */  
  TrgSumData     TrgSum;                /* Summary data: 480 bytes */
  RawTrgDet      rawTriggerDet[MAX_RAW_DATA_BLOCKS];    /* Raw Detector Data with pre and post History: 11*1792 bytes */
} TrgDataType;         /* 20232 bytes */
