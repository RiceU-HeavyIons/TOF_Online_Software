...
int ssdReader(char *m)
{
	...
	if(checkBank((char *)ssdp,"SSDP") < 0) return EVP_DATA_ERR ;	// wrong bank!
	...
	for(sec=0;sec<1;sec++) {	// just one sector! I kept the for loop for compatibility
		...
		if(checkBank((char *)secp,"SSDSECP") < 0) return EVP_DATA_ERR ;
		...
		for(rb=0;rb<last;rb++) {
			...
			for(mz=0;mz<2;mz++) {	// 3rd mezzanine will be always empty...
				...
				if(mzp->banks[TPC_PADK].len != 0) {	
					...
					if(checkBank((char *)padk,"SSDPADK") < 0) return EVP_DATA_ERR ;
				...
				if(mzp->banks[TPC_CPPR].len != 0) {
					...
					if(checkBank((char *)cppr,"SSDCPPR") < 0) return EVP_DATA_ERR ;
				}

				if((mzp->banks[TPC_ADCR].len != 0) && (ssd.mode==0)) {	// raw ...
					adcr = (struct TPCADCR_l *)((u_int *)mzp + l2h32(mzp->banks[TPC_ADCR].off)) ;

					if(checkBank((char *)adcr,"SSDADCR") < 0) return EVP_DATA_ERR ;
				...
				if(mzp->banks[TPC_PEDR].len != 0) {	// pedestal data!
					...
					if(checkBank((char *)pedr,"SSDPEDR") < 0) return EVP_DATA_ERR ;
				...
				if(mzp->banks[TPC_RMSR].len != 0) {	// RMS too 
					...
					if(checkBank((char *)rmsr,"SSDRMSR") < 0) return -1 ;
