...
static fcfClass *fcf = NULL;
static fcfAfterburner *fcf_after = NULL;
...
// 
int fcfReader(int sector, int *t0c, u_int *gainc)          // from zero
{
  int row, p, t;
  u_short tb;
...
// 	}
	...
	if(adc[p][tb] > 0) {
	  ...
	  if(cou <= 60)          // more than 31 clusters?
	    ...
	    LOG(WARN, "Cou is begger %d pad %d tb %d",cou,p,tb);
	}
	...
	if(started && (adc[p][tb] == 0)) {  // finish cluster
	  ...
	  if(cou <= 61) 
	    ...
	    LOG(WARN, "Cou is bigger %d pad %d tb %d",cou,p,tb);
