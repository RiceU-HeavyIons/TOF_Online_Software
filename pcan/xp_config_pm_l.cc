...
"$Id: xp_config_pm_l.cc,v 1.3 2011-10-31 15:02:47 jschamba Exp $";
#endif /* lint */
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_pm_l: Sending CONFIGURE_TDC:Block Start command:");
#endif

  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_pm_l: CONFIGURE_TDC:Write Block Start", 4000000,2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "xp_config_pm_l: Sending CONFIGURE_TDC:Block DATA packet:");
#endif
    
    
    if ( sendCAN_and_Compare_l(devID, ms, "xp_config_pm_l: CONFIGURE_TDC:Block DATA", 
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_pm_l: Sending CONFIGURE_TDC:Block DATA packet 12:");
#endif
  
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_pm_l: CONFIGURE_TDC:Block DATA 12:", 
			     4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_pm_l: Sending CONFIGURE_TDC:Write Block End packet:");
#endif


  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_pm_l: CONFIGURE_TDC:Write Block End:", 
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_pm_l: Sending CONFIGURE_TDC:Write Block Target Config PM packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_pm_l: CONFIGURE_TDC:Write Block Target Config PM:", 
			     4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
