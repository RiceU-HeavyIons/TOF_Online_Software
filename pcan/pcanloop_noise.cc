...
"$Id: pcanloop.cc 686 2012-01-19 18:30:08Z jschamba $";
#endif /* lint */
  ...
  while (1) {	//**************** big while loop here ************************
	...
	if (strlen(txt) != 1) {
	  ...
	  if (fp != (FILE *)NULL)
	    ...
	    string stRuntimeName("runtimes");

	    if(stFilename.find_last_of("/") != string::npos) {
	      stRuntimeName.insert(0,stFilename.substr(0,stFilename.find_last_of("/")+1));
	      struct stat st;
	    ...
	    if ((rtfp = fopen (stRuntimeName.c_str(), "w")) == (FILE *)NULL) {
	      cout << "Couldn't open file " << stRuntimeName << endl;
	    }
	    else {
	      fprintf(rtfp, "%d\n", (int)time(theTime));
	    }
