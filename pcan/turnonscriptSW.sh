...
for ((i=21; i<40; i+=1)); do
    ./pc "m s 0x"$i"2 5 0x8a 0x69 0x96 0xa5 0x5a 255"
    usleep 70000
    for j in 40 44 48 4c 50 54 58 5c; do
	./pc "m e 0x"$j"800"$i" 5 0x8a 0x69 0x96 0xa5 0x5a 255"
	usleep 70000
    done
done
...
#	./xsetThreshold 255 0x"$j" 0x"$i" 2500
	./xsetThreshold 255 0x"$j" 0x"$i" 1200
	usleep 70000
    done
    ./pc "m s 0x"$i"2 3 0xe 0x2 0xf 255"
    usleep 70000
    # set mult gate delay
    ./pc "m s 0x"$i"2 3 0xe 0x8 0xe0 255"
    usleep 70000
done
