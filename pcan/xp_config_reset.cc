...
"$Id: xp_config_reset.cc,v 1.2 2011-10-31 14:33:45 jschamba Exp $";
#endif /* lint */
...
int p_config(const char *filename, unsigned int tdigNodeID, unsigned int tcpuNodeID, int TDC, WORD devID)
{
  ...
  // ****************** Control Word: PLL_reset=1, DLL_reset=1 **********************
  ms.LEN = 6;
  ms.DATA[0] = 0x4 | (TDC & 0x3);
  ms.DATA[1] = 0xe4;
  ms.DATA[2] = 0xff;
  ...
  if ( sendCAN_and_Compare(ms, "xp_config: Control-PLLrst = DLLrst = 1:", 4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);

  // ****************** Control Word: PLL_reset=0, DLL_reset=1 **********************
  // ms.LEN = 6;
  // ms.DATA[0] = 0x4 | (TDC & 0x3);
  // ms.DATA[1] = 0xe4;
  // ms.DATA[2] = 0xff;
  ...
  if ( sendCAN_and_Compare(ms, "xp_config: Control-PLLrst = 0 DLLrst = 1:", 4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);

  // ****************** Control Word: PLL_reset=0, DLL_reset=0 **********************
  // ms.LEN = 6;
  // ms.DATA[0] = 0x4 | (TDC & 0x3);
  // ms.DATA[1] = 0xe4;
  // ms.DATA[2] = 0xff;
  ...
  if ( sendCAN_and_Compare(ms, "xp_config: Control-PLLrst = 0 DLLrst = 0:", 4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);

  // ****************** Control Word: global reset **********************
  // ms.LEN = 6;
  // ms.DATA[0] = 0x4 | (TDC & 0x3);
  ms.DATA[1] = 0xf4;
  // ms.DATA[2] = 0xff;
  ...
  if ( sendCAN_and_Compare(ms, "xp_config: Control-global reset:", 4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);

  // ****************** Control Word: enable all **********************
  // ms.LEN = 6;
  // ms.DATA[0] = 0x4 | (TDC & 0x3);
  ms.DATA[1] = 0xe4;
  // ms.DATA[2] = 0xff;
