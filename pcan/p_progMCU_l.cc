...
"$Id: p_progMCU_l.cc,v 1.1 2008-11-06 21:22:14 jschamba Exp $";
#endif /* lint */
		 ...
		 )
{
  ...
  if ( (errno = CAN_Write_l(h, &ms, devID)) ) {
    perror("p_progMCU_l: CAN_Write_l()");
    return(errno);
  }
  ...
  ms.DATA[1] = *tmpPtr;
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "p_progMCU_l: Sending progMCU:Program64 command:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "p_progMCU: progMCU:Program64", 1000000) != 0) { // timeout = 1 sec
...
int change_mcu_program(const char *filename, unsigned int nodeID, WORD devID)
{
  ...
  while (hexfile.good()) {
    ...
    */
	...
	if (endAddr > startAddr) {
	  ...
	  if( send_64bytes(pgmByte, startAddr, checksum_64, nodeID, devID) != 0)
	    my_private_exit(errno);
	}
...
#endif
	    if( send_64bytes(pgmByte, tmpAddr, checksum_64, nodeID, devID) != 0)
	      my_private_exit(errno);
	  }
  ...
  errno = 0;
  if ( sendCAN_and_Compare_l(devID, ms, "CHANGE_MCU_PROGRAM:Final_chksum", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
  ...
  errno = 0;
  sendCAN_and_Compare_l(devID, ms, "CHANGE_MCU_PROGRAM:Jump_PC", 5000000); // timeout = 5 sec
