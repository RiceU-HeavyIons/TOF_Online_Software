/* File name     : xp_config_scripted_l.cc
...
"$Id: xp_config_scripted_l.cc,v 1.2 2011-10-31 14:38:24 jschamba Exp $";
#endif /* lint */
...
#ifdef LOCAL_DEBUG
  cout << "xp_config_scripted_l: finished (" << error << ")\n";
#endif
  exit(error);
}
...
int p_config(const char *filename, unsigned int tdigNodeID, unsigned int tcpuNodeID, int TDC, WORD devID)
{
  ...
  ms.DATA[5] = 0x00;
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Block Start command:");
#endif

  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: Control-Power Down:", 
			     4000000, 2, true) != 0) // timeout = 4 sec
  ...
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Write Block Start", 
			     4000000,2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Block DATA packet:");
#endif
    
    
    if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Block DATA", 
			       4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Block DATA packet 12:");
#endif
  
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Block DATA 12:", 
			     4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Write Block End packet:");
#endif


  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Write Block End:", 
			     4000000, 8, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Write Block Target TDC packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Write Block Target TDC:", 
			     4000000, 2, true) != 0) // timeout = 4 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Start command:");
#endif

  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Start", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:DATA packet:");
#endif
    
    
    if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:DATA", 1000000) != 0) // timeout = 1 sec
      my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:DATA packet 12:");
#endif
  
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:DATA 12:", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Config_end packet:");
#endif


  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Config_end:", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending CONFIGURE_TDC:Program packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: CONFIGURE_TDC:Program:", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending DEBUG:TDC_Reset_Assert packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: DEBUG:Assert_TDC_Reset", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending DEBUG:TDC_Reset_Deassert packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: DEBUG:Deassert_TDC_Reset", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "xp_config_scripted_l: Sending DEBUG:PLD_Reset packet:");
#endif
  
  if ( sendCAN_and_Compare_l(devID, ms, "xp_config_scripted_l: DEBUG:PLD_Reset", 1000000) != 0) // timeout = 1 sec
    my_private_exit(errno);
