...
"$Id: p_eeprom2_l.cc,v 1.2 2011-10-31 14:51:34 jschamba Exp $";
#endif /* lint */
...
#ifdef LOCAL_DEBUG
  cout << "p_eeprom2_l: finished (" << error << ")\n";
#endif
  exit(error);
}
...
int eeprom2(const char *filename, unsigned int nodeID, WORD devID)
{
  ...
  for (int page=0; page<noPages; page++) {
    ...
    if ( sendCAN_and_Compare_l(devID, ms, "p_eeprom2: Write Block Start", 4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
	...
	tmpPtr++;
      ...
      if ( sendCAN_and_Compare_l(devID, ms, "p_eeprom2: Write Block Data", 4000000, 2, true) != 0) // timeout = 4 sec
	my_private_exit(errno);
    ...
    if ( sendCAN_and_Compare_l(devID, ms, "p_eeprom2: Write Block Data 37:", 4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
    ...
    if ( sendCAN_and_Compare_l(devID, ms, "p_eeprom2: Write Block End:", 4000000, 8, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
    ...
    if ( sendCAN_and_Compare_l(devID, ms, "p_eeprom2: Write Block Target:", 4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
