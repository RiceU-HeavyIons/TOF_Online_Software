...
int getStatus(int tdcNum, int nodeID)
{
  FILE *fp;
  char txt[256];
  int fifofd;
 ...
 // send the "GET_STATUS" CANbus HLP message
  ...
  if (numRead > 0) {
    ...
    for (int i=1; i<m.LEN; i++) {
      /* For TDIG-D we are sending LSByte first */
      dat = (int)m.DATA[i];
      for (int k=0; k<8; k++) {
  ...
  if (bitNum > 60) {
    // format the output
    fprintf(stdout,  "Status reply for Board (node) %d, TDC # %d\n", nodeID, tdcNum);
...
int main(int argc, char *argv[])
{
  ...
  return getStatus(tdcNum, nodeID);
}
