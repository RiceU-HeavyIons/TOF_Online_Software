...
"$Id: xMCU2_l.cc,v 1.2 2011-10-31 15:01:42 jschamba Exp $";
#endif /* lint */
		     ...
		     )
{
  ...
  if (bytecount != 0) {
    ...
    if ( sendCAN_and_Compare_l(devID, ms, "MCU2:BlockStart", 4000000, 2, true) != 0) { // timeout = 4 sec
	...
	tmpPtr++;
      ...
      if ( sendCAN_and_Compare_l(devID, ms, "MCU2:BlockData", 4000000, 2, true) != 0) { // timeout = 4 sec
	my_private_exit(errno);
      }
...
#endif
    if ( sendCAN_and_Compare_l(devID, ms, "MCU2:BlockTargetMCU2", 4000000, 2, true) != 0) { // timeout = 4 sec
      my_private_exit(errno);
    }
  ...
  while (hexfile.good()) {
    ...
    */
    ...
    if(rtype == 0x4) { // linear address record
      ...
      if (downloadbytes != 0) {
	xwrite_mcu_block(dataByte, baseAddress, downloadbytes, eraseflag, msgIDVal, devID);
