...
"$Id: eeprom2.cc 756 2012-07-05 20:44:12Z jschamba $";
#endif /* lint */
...
// here all is done
//*********************************************
int eeprom2(const char *filename, unsigned int nodeID, int devID)
{
  int i,j;
...
#ifdef LOCAL_DEBUG
  printCANMsg(ms, "eeprom2: Sending Write FPGA_CONFIG0 command:");
#endif
  
  if ( sendCAN_and_Compare(h, ms, "eeprom2: Write FPGA_CONFIG0", 500000, 2, true) != 0) // timeout = 0.5 sec
    printf("Old MCU Code?\n");
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "eeprom2: Sending Write Block Start command:");
#endif
    
    if ( sendCAN_and_Compare(h, ms, "eeprom2: Write Block Start", 4000000, 2, true) != 0) // timeout = 4 sec
...
#ifdef LOCAL_DEBUG
      printCANMsg(ms, "eeprom2: Sending Write Block Data packet:");
#endif
      
      
      if ( sendCAN_and_Compare(h, ms, "eeprom2: Write Block Data", 4000000, 2, true) != 0) // timeout = 4 sec
	my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "eeprom2: Sending Write Block Data packet 37:");
#endif
    
    
    if ( sendCAN_and_Compare(h, ms, "eeprom2: Write Block Data 37:", 4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "eeprom2: Sending Write Block End packet:");
#endif
    
    
    if ( sendCAN_and_Compare(h, ms, "eeprom2: Write Block End:", 4000000, 8, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
...
#ifdef LOCAL_DEBUG
    printCANMsg(ms, "eeprom2: Sending Write Block Target packet:");
#endif
    
    if ( sendCAN_and_Compare(h, ms, "eeprom2: Write Block Target:", 4000000, 2, true) != 0) // timeout = 4 sec
      my_private_exit(errno);
